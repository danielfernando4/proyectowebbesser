...
async def create_library(library_data: LibraryCreate, database: Session = Depends(get_db)) -> Library:
    ...
    db_library = Library(
        telephone=library_data.telephone,        name=library_data.name,        address=library_data.address,        web_page=library_data.web_page        )
...
async def bulk_create_library(items: list[LibraryCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_library = Library(
                telephone=item_data.telephone,                name=item_data.name,                address=item_data.address,                web_page=item_data.web_page            )
            database.add(db_library)
            database.flush()  # Get ID without committing
...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    db_book = Book(
        title=book_data.title,        release=book_data.release,        genre=book_data.genre.value,        stock=book_data.stock,        pages=book_data.pages,        price=book_data.price        )
...
async def bulk_create_book(items: list[BookCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_book = Book(
                title=item_data.title,                release=item_data.release,                genre=item_data.genre.value,                stock=item_data.stock,                pages=item_data.pages,                price=item_data.price            )
            database.add(db_book)
            database.flush()  # Get ID without committing
